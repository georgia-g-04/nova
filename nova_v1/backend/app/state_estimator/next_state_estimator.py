...
from schemas.user_state import UserState, NextStatePrediction
...
You will receive input in the format of {UserState.model_json_schema()}. Your job is to look at the current user state and 
...
Assign the predicted next user state to inferred_next_user_state and confidence to next_state_confidence in the output schema. 
...
def estimate_user_state(user_input):
    response = client.responses.parse(
        ...
        text_format = NextStatePrediction,
        input = user_input
    )
